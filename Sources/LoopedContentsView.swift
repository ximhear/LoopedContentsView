...
public class LoopedContentsView: UIView, UIScrollViewDelegate {
    ...
    override init(frame: CGRect) {
        ...
        self.setScrollPosition(position: 0, animated: true)
    }
    ...
    public var cellTransform: ((_ range: CGFloat) -> CGAffineTransform)? = nil
    
    public var cellAlpha: ((_ range: CGFloat) -> CGFloat)? = nil
    ...
    public var scrollPosition : CGFloat {
        ...
        set {
            self.setScrollPosition(position: newValue, animated: false)
        }
    }
    ...
    public func selectItem(atIndex index: Int, animated: Bool) {
        ...
        if let delegate: LoopedContentsViewDelegate = self.delegate {
            delegate.loopedContentsView(loopedContentsView: self, willSelectContentAtIndex: index)
        }
        ...
        if let delegate: LoopedContentsViewDelegate = self.delegate {
            delegate.loopedContentsView(loopedContentsView:self, didSelectContentAtIndex: index)
        }
    }
    ...
    public func deselectItem(atIndex index: Int, animated: Bool) {
        ...
        // Call Delegate Method (Will)
        if let delegate: LoopedContentsViewDelegate = self.delegate {
            delegate.loopedContentsView(loopedContentsView:self, willDeselectContentAtIndex: index)
        }
        ...
        // Call Delegate Method (Did)
        if let delegate: LoopedContentsViewDelegate = self.delegate {
            delegate.loopedContentsView(loopedContentsView:self, didDeselectContentAtIndex: index)
        }
    }
    ...
    public func reloadData() {
        ...
        self.numberOfItems = self.dataSource?.loopedContentsViewNumberOfContents(loopedContentsView: self) ?? 0
        self.lengthOfItems = (0..<self.numberOfItems).map { (index: Int) -> CGFloat in
            return self.delegate?.loopedContentsView(loopedContentsView:self, lengthOfContentAtIndex: index) ?? 0.0
        }
    ...
    private func updateVisibleCells() {
        ...
        for (_index, itemIndex) in itemIndexes.enumerated() {
            ...
            if newCellIndexSet.contains(index) {
                ...
                let cell: LoopedContentsViewCell = dataSource.loopedContentsView(loopedContentsView:self, cellAtIndex: itemIndex)
                ...
                cell.setSelected(selected: selected, animated: false)
                
                let tapGestureRecognizer: UITapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(onCellTapped(sender:)))
                cell.addGestureRecognizer(tapGestureRecognizer)
                
                // Cell Will Display
                if let delegate: LoopedContentsViewDelegate = self.delegate {
                    delegate.loopedContentsView(loopedContentsView:self, willDisplayCell: cell, forItemAtIndex: itemIndex)
                }
        ...
        for index in disableCellIndexSet {
            if let cell = self.activeCells[index] {
                ...
                if let delegate: LoopedContentsViewDelegate = self.delegate {
                    delegate.loopedContentsView(loopedContentsView: self, didEndDisplaying: cell, forItemAtIndex: cell.itemIndex)
                }
    ...
    public func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        ...
        if let delegate: LoopedContentsViewDelegate = self.delegate, let cell: LoopedContentsViewCell = self.activeCells[self.centerItem.index] {
            delegate.loopedContentsView(loopedContentsView: self, didFocusCell: cell, forItemAtIndex: centerItem.itemIndex)
        }
